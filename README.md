...
- **写真の拡大（トップ・日別詳細 共通）** — ★**拡大は `#lb-img` の transform で自前に持つ**
  （ピンチ・1本指パン・ダブルタップで 1x⇄2.5x）。ページ側のズームは例外なく禁止
  （`#lb { touch-action: none }` ＋ gesture イベントを常時 preventDefault）。
  ブラウザ任せのピンチが拡大するのは img ではなく**ページ全体**なので、閉じても倍率だけが残り、
  下の日別詳細ページが拡大されたままになる。**閉じる瞬間に meta viewport を `maximum-scale=1` に
  する回避策は iOS 実機で不発**（ユーザー操作でかけた倍率は戻らない）。自前で持てば
  「閉じる＝transform を捨てる」だけで確実に戻る。
  指を動かした直後の click は window の capture で握り潰す（パン終わりで閉じないため）。
  期限付き（400ms）にするのは、ピンチ後に click が来ない端末でフラグが残り、
  次の本物のタップを1回食べるのを避けるため

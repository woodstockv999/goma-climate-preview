...
- **日別詳細** — 行動内訳（24枠）＋グラフ2枚。℃ と % はスケールが違うので1枚に2軸を立てない。センサー停止時だけ1行の注意書き。
  行動内訳は**割合で引き伸ばす棒をやめて24枠の時間割に**した。棒は時間軸を持たないので
  真下の室温グラフと時刻が対応せず、「12時に暑かった時、何をしていたか」が読めなかった。
  ★**横位置を厳密に揃える3点**（1つ欠けてもずれる。実測で全枠 0.02px 以内）:
  ①`.act-plot` に `.chart-card` と**同じ箱を透明で被せる**（`border:1.5px solid transparent`
  ＋`padding:0 0.3rem`）。**数値で足してはいけない** — border-width はブラウザがデバイス
  ピクセルへ丸めるので（`1.5px` が dsf=1 では 1px になる）、同じ指定で同じ丸めを受けさせる。
  その内容幅＝SVG幅に対して `margin-left:7.647%` / `margin-right:2.941%`（= `pL`/`pR`）
  ②`.act-strip` の左右 padding は gap の**半分**（1px）— 2px だと枠の中央が最大1pxずれる
  ③グラフの X は**枠の中央**へ（`X(h) = pL + iw*(h+0.5)/24`・目盛りは境界 `pL + iw*t/24`）

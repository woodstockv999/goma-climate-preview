...
- **写真の拡大（トップ・日別詳細 共通）** — ページのピンチズームは常時禁止し、ライトボックス
  （`#lb`）表示中だけ解放する。★閉じるときは `closeLightbox()` を通し、**meta viewport を一瞬
  `maximum-scale=1` にして倍率を1へ戻す**。ピンチが拡大しているのは img ではなく**ページ全体**なので、
  `display:none` にするだけだと元の日別詳細ページが拡大されたまま残る。
  戻したあと元の content に復帰させること（しないと次に開いたときピンチできない）

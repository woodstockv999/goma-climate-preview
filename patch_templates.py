...
CLIMATE_CSS = """
    ...
    /* スパークラインは viewBox 幅240を約344pxへ拡大表示するため、日別詳細のグラフ
       ...
       non-scaling-stroke で線幅を画面実寸に固定し、小さい図に合う細さにする。 */
    .spark .series-line, .spark .thresh-line, .spark circle {
      vector-effect: non-scaling-stroke;
    }
    .spark .series-line { stroke-width: 1.4; }
    .spark circle { stroke-width: 1.2; }
    ...
    /* opacity をここで指定してはいけない。CSS は presentation attribute に勝つので、
       ...
       縦線が出たままになる。表示・非表示は JS 側の属性だけで制御する。 */
    .cross-line { stroke: var(--text); stroke-width: 1; }
    .chart-tip {
